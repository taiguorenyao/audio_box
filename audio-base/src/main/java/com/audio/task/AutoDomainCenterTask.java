...
public class AutoDomainCenterTask implements ServletContextListener
{
	...
	static void task(ServletContext context)
    {
        ...
        timer.scheduleAtFixedRate(new TimerTask()
        {
            ...
            public void run()
            {
                try
                {
                   System.out.println("一分钟执行一次(延迟10秒)");
                   //检查域名
                   ...
                   if (domain != null)
                   {
                	 ...
                	 //查询当前使用域名
                       ...
                       {
                    	   //更新检查成功日志
                           Manager.invoke().userManager(context).updateObjectData(21,domain.getId(),"【"+
                       ...
                       else
                       {
                           ...
                           {
                               //记录没有替换的原因
                               Manager.invoke().userManager(context).updateObjectData(21,domain.getId(),"【原因：因无状态为2的备用域名使用，所以当前域名未替换。【"+DateTimeUtil.getNow(DateTimeUtil.FORMAT_LONG)+"】】"+logResult);
                               return;
                           }
                           ...
                           Manager.invoke().userManager(context).updateObjectData(22,"1",domain.getId(),"【"+DateTimeUtil.getNow(DateTimeUtil.FORMAT_LONG)+"】"+logResult);


                           //更新config表Domain字段 然后刷新缓存
                           Manager.invoke().userManager(context).updateObjectData(17,"AVI_URL","http://"+bydomain.getVal()+"/");
                           ...
                           //备用域名没问题，执行备用域名状态为 =0
                           Manager.invoke().userManager(context).updateObjectData(23,"0",bydomain.getId(),"【"+DateTimeUtil.getNow(DateTimeUtil.FORMAT_LONG)+"】域名替换成功！",resMsg);
