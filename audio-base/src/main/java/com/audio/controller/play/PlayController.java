...
//                RedisUtils.BASE_CONFIG + "AVI_URL");
            ...
            HttpServletRequest req, HttpServletResponse resp) throws IOException
    {
          ...
          //解密
          String id = uid;
          BoxUser mdUser = MD5Util.mdUserMap.get(id);
          if (null == mdUser)
          {
              BoxUser newMdUser = (BoxUser)userService.loadDate(11,id);
              if (newMdUser != null)
              {
                  MD5Util.mdUserMap.put(uid,newMdUser);
                  uid = newMdUser.getId();
              }
          }
          else
        ...
        //随机获取一个播放
        List<PersonAviEvt> aviList =  CacheConfig.getPerMp4Map(uid);
        //如果用户不存在没有获取到数据则跳转到指定地址
//        if (null == aviList || aviList.isEmpty())
//        {
//            resp.sendRedirect(
//                    "https://news.html5.qq.com/share/2000121624?ch=060000&tabId=0&tagId=0&docId=2000121624&url=http%3A%2F%2Fkuaibao.qq.com%2Fs%2F20170603A01QEQ00&clientWidth=360&dataSrc=&qburl=qb%3A%2F%2Fext%2Fread%3Fcid%3D0%26type%3D0%26mttsummaryid%3D2000121624%26b_f%3D060000%26bizid%3D1&sc_id=yVIbIEC");
//            return null;
...
//        model.addAttribute("playAvi", playAvi);
        ...
        //记录访问记录
        Config data_config = CacheConfig.getConfigMap("DATA_ACCESS_STATUS");
        //配置项为1才记录
        if (null != data_config && "1".equals(data_config.getValue()))
        {
            AcessLog acessLog = new AcessLog();
            //acessLog.setIp(StringUtil.getRequestIp(req));
            acessLog.setIp(req.getRemoteAddr());
            acessLog.setUid(uid);
            try
            {
                //解决mysql崩溃时放弃统计。
