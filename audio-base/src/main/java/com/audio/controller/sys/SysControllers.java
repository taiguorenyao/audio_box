...
public class SysControllers
{
    ...
    private PaginationSupport<AcessStatistical> paginationSupport_click = new PaginationSupport<AcessStatistical>();
...
//                    config);
            ...
            Model model, HttpServletRequest req)
    {
        ...
        if ("315_URL_S".equals(config.getKey()) &&
...
//                    config);
    ...
    public String refreshDomainCenter(@PathVariable String id,
            HttpServletRequest req)
    {
    ...
    @RequestMapping("/clicklist")
    public String clicklist(Model model, HttpServletRequest req,
            Integer curPage,
            Integer pageSize, String startTime, String endTime)
    {
        ...
        {
            startTime = DateTimeUtil.getNow(DateTimeUtil.FORMAT_SHORT) +
                    " 00:00:00";
            endTime = DateTimeUtil.getNow(DateTimeUtil.FORMAT_SHORT) +
                    " 23:59:59";
        }
        else
        {
            if (startTime.indexOf("00:00:00") <= 0)
            {
                startTime += " 00:00:00";
                endTime += " 23:59:59";
            }
        ...
        paginationSupport_click = dataService.getClickList(curPage,
                pageSize, startTime, endTime);

//        //重组数据
//        if (paginationSupport_click != null && paginationSupport_click.getItems() != null)
//        {
//            List<AcessStatistical> list = paginationSupport_click.getItems();
//            if (!list.isEmpty())
//            {
//                //放入MAP相加
//                Map<String,AcessStatistical> siteMaps = new HashMap<String,AcessStatistical>();
//
//                for (AcessStatistical access : list)
//                {
//                    AcessStatistical base= siteMaps.get(access.getUid());
//                    if (null == base)
//                    {
//                        siteMaps.put(access.getUid(),access);
//                    }
//                    else
//                    {
//                        BigDecimal baseDec = new BigDecimal(base.getAccessCount());
//                        BigDecimal lDec = new BigDecimal(access.getAccessCount());
//                        base.setAccessCount(baseDec.add(lDec).toString());
//                        siteMaps.put(base.getUid(),base);
//                    }
//                }
//
//                List<AcessStatistical> resultList = new ArrayList<AcessStatistical>();
//                for(Object obj : siteMaps.keySet())
//                {
//                    resultList.add((AcessStatistical)obj);
//                }
//                paginationSupport_click.setItems(resultList);
//            }
//        }

        model.addAttribute("paginationSupport", paginationSupport_click);
        model.addAttribute("curPage", curPage);
        model.addAttribute("startTime", startTime);
        model.addAttribute("endTime", endTime);
        return "sys/click/list";
    }

    @RequestMapping("/mdcode")
    public String mdcode(BoxUser boxUser,
            HttpServletRequest req)
    {
        paginationSupport_user = userService.getUserList(1,
                5000, "");

        List<BoxUser> ulist = paginationSupport_user.getItems();
        int count = 0;
        for (BoxUser user : ulist)
        {
            user.setMd5Code(MD5Util.string2MD5(user.getId()));
            boolean result = userService.updateObjectData(28, user);
            count++;
        }

        StringUtil.setErrorCode(req, "成功处理【"+count+"】条记录!");
