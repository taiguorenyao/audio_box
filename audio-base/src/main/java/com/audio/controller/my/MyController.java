...
public class MyController
{
    ...
    public String addAudio(Model model, PersonAviEvt data,
            HttpServletRequest req)
    {
        Config config_ = CacheConfig.getConfigMap("DISABLED_MP4");
        if (null != config_)
        {
            model.addAttribute("disabled_mp4", config_.getValue());
        }
    ...
    public String myMember(Model model, HttpServletRequest req)
    {
        ...
        List<BoxUser> data = (List<BoxUser>) userService.loadAllDate(8,
                boxUser.getAccount());
    ...
    public String addAudiodo(PersonAviEvt data, HttpServletRequest req)
    {
        ...
        if (data.getUrl().indexOf("@") >= 0)
        {
            ...
            for (int i = 0; i <= (urls.length - 1); i++)
            {
                ...
                //是否判断是否只允许上传腾讯视频
                if ("1".equals(config_.getValue()))
                {
                    //如果非腾讯地址跳过
                    if (!StringUtil.isTX(u))
                    {
                        counts++;
                        continue;
                    }
        ...
        else
        {
            if (!StringUtil.isEmpty(StringUtil.cleanXSS(data.getUrl())))
            {
                ...
                {
                    //如果非腾讯地址跳过
                    if (StringUtil.isTX(StringUtil.cleanXSS(data.getUrl())))
                    {
        ...
        if ("1".equals(config_.getValue()))
        {
            StringUtil.setErrorCode(req, "添加成功,自动过滤非腾讯视频" + counts + "条!");
        }
        else
        {
    ...
    {
        String result = CaheBlog.refreshNode("/cache/refreshPlayMp4",
                "uid=" + uid);
            ...
            String nodata, PersonAdEvt personAdEvt)
    {
        ...
        if (null == perList || perList.isEmpty())
        {
            ...
            if (result)
            {
                String msg = CaheBlog.refreshNode("/cache/refreshPlayPersonAd",
                        "uid=" + boxUser.getId());
...
//                        personAdEvt);
    ...
    {
        String result = CaheBlog.refreshNode("/cache/refreshPlayAd",
                "uid=" + uid);
        StringUtil.setErrorCode(req, result);
        return "redirect:/index";
    }
    ...
    public String sinaUrl(String uid, HttpServletRequest req)
    {
        ...
        for (int i = 0; i <= 19; i++)
        {
            ...
            //缩短返回
            urls = urls + "/play/cen/"+MD5Util.string2MD5(uid);
            ...
            Integer pageSize)
    {
        ...
        BoxUser boxUser = SessionUtil.getPortalUser(req);
        //根据登录upuser uid获取 uid
        List<BoxUser> uidlist = (List<BoxUser>) userService.loadAllDate(8,
                boxUser.getAccount());
        ...
        if (uidlist != null)
        {
            ...
            // String uid = userService.loadData
            String startTime = DateTimeUtil.getNow(DateTimeUtil.FORMAT_SHORT) +
                    " 00:00:00";
            String endTime = DateTimeUtil.getNow(DateTimeUtil.FORMAT_SHORT) +
                    " 23:59:59";

            paginationSupport_click = dataService.getMyClickList(curPage,
                    pageSize, startTime, endTime, uid);
