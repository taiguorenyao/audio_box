...
public class AESEncrypter
{
    ...
    public static void main(String[] atr) throws UnsupportedEncodingException
    {
        //20170629947119 20170605215840
        String s = jiami("201784744238412");
        System.out.println(s);
        System.out.println(jiemi(s));
        String pwd = AESEncrypter.getInstance().decrypt("NTr/ivme2DWrCQT1DEMuYw==",
                AESEncrypter.VOCE);
        System.out.println(pwd);

    }

    public static String jiami(String val)
    {
        String s = AESEncrypter.getInstance().encrypt(val, AESEncrypter.VOCE);
        s = s.replaceAll("=", "");
        s = s.replaceAll("/", "@");
    ...
    public static String jiemi(String val)
    {
        val = val.replaceAll("@", "/");
        //解决新浪短地址+被强制为%20问题
        val = val.replaceAll(" ", "+");
        ...
        try
        {
            s1 = AESEncrypter.getInstance().decrypt(val, AESEncrypter.VOCE);
